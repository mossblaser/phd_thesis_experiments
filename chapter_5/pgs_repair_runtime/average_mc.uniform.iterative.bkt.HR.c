...
bool a_star_required = false; // set when a dead link is routed through and the
...
// The number of faults to try
#define FROM_PERCENTAGE(perc) ((long)((256*256*3.0) * ((perc) / 100.0)))
const long faults[] = {FROM_PERCENTAGE(-1.0), // XXX
                       FROM_PERCENTAGE(0.00),
                       FROM_PERCENTAGE(0.01),
...
// The number of destinations to try
const long dests[] = {16};
#define NUM_DEST_VALUES (sizeof(dests)/sizeof(dests[0]))

long double resrc[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//resources
long double  ent[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//entries
long double useX[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//dim X
long double useY[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//dim Y
long double useD[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//dim Z
long double exec[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//exec. time
long double post_exec[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//postprocessing exec. time
long double disconnected[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ; //disconnected nodes

long double        resrc_min[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//resources
long double          ent_min[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//entries
long double         useX_min[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//dim X
long double         useY_min[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//dim Y
long double         useD_min[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//dim Z
long double         exec_min[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//exec. time
long double    post_exec_min[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//post-processing exec. time
long double disconnected_min[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ; //disconnected nodes

long double        resrc_max[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//resources
long double          ent_max[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//entries
long double         useX_max[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//dim X
long double         useY_max[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//dim Y
long double         useD_max[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//dim Z
long double         exec_max[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//exec. time
long double    post_exec_max[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//post-processing exec. time
long double disconnected_max[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ; //disconnected nodes


long double        resrc_sq[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//resources
long double          ent_sq[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//entries
long double         useX_sq[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//dim X
long double         useY_sq[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//dim Y
long double         useD_sq[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//dim Z
long double         exec_sq[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//exec. time
long double    post_exec_sq[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//post-processing exec. time
long double disconnected_sq[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ; //disconnected nodes

long double        resrc_stdev[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//resources
long double          ent_stdev[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//entries
long double         useX_stdev[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//dim X
long double         useY_stdev[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//dim Y
long double         useD_stdev[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//dim Z
long double         exec_stdev[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//exec. time
long double    post_exec_stdev[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ;//post-processing exec. time
long double disconnected_stdev[NUM_DEST_VALUES][NUM_FAULT_VALUES][5] ; //disconnected nodes
 ...
 */
void _a_star_reconnect(long px, long py, long x, long y) {
	...
	// XXX: We assume that no disconnected regions exist so a path should
	// always be available.
	if (!path_found){
		printf("not found from %d %d to %d %d\n", x, y, px, py);
	}
	assert(path_found);
...
void a_star(void) {
	...
	while (a_star_disconnections) {
		...
		a_star_cur_visit_num++;
		_a_star_reconnect(disconnection->px, disconnection->py,
...
void just_a_star(long f) {
	...
	long i, bkt, x, y;
	struct a_star_disconnection_list *disconnection;
	a_star_required = false; // We won't need to patch up these routes later!
 ...
 */
void create_faults(long faults) {
	long x, y, k, hop;
	long ax, ay, pt;
	...
	while (!fully_connected_map) {
		for (x=0; x<341; x++)
			for (y=0; y<341; y++){
				map[x][y]=-1; // Set to 0 by flood-fill
				for (k=0; k<6; k++)
    	            fault[x][y][k]=0;
			}
			...
			while (k > 0) {
				...
				ax=((rand()%(256 / 12)) * 12) + 43;
				ay=((rand()%(256 / 12)) * 12) + 43;
...
long double create(long f, long faults){
	long dst, total=0;
	long i,j,k,bkt;
...
//printf("3.put nodes\n");fflush(NULL);
	...
	for (i=0;i<f;i++) {
		if ((r=rand()%100) < 5*cent){
			...
			//	printf("1.\n");fflush(NULL);
				if (dst>15 || dst<=0)
					dst=15;  // for aprox. 2K (10%)// should be 16+ for 8K
			//	printf("2.\n");fflush(NULL);
				switch (rand()%6) {
				case 0: //+X -Y
					pt=rand()%(dst+1);
			...
			do {
				...
				if (dst>30 || dst<=0)
					dst=30;  // for aprox. 2K // should be 50+ for 8K
			//	printf("s: %d\n",dst);
                bkt=dst/BKT_LEN;
				while(bucket[bkt]>=BKT_SIZE){
					bkt=(bkt+BKT_NUM-1)%BKT_NUM;    // == (bkt-1)%13, but returns a positive number
...
// Maximum distance to look around in smart
int main(int argc, char *argv[]){
	long ax, ay, pt;
	long i,j,k,t;
	long r;
	struct timeval  start_time;	///< Simulation start time / date.
	struct timeval  end_time;	///< Simulation finish time / date.
	struct timeval  post_end_time;	///< Postprocess finish time / date.

	long start_rep=0; // Repeat number (i.e. seed) to start on
	long end_rep=REP; // Repeat number (i.e. seed) to end before
	// Optionally accept the range of repeats to run on the commandline as an
	// argument
	if (argc == 3) {
		start_rep = atol(argv[1]);
		end_rep = atol(argv[2]);
	} else if (argc != 1) {
		fprintf(stderr, "ERROR: Expected either no arguments or two arguments "
		                "(start_rep and end_rep).\n");
		return 1;
	}
	long rep=end_rep - start_rep; // Number of repeats in this run

	//initialise statistics to 0
	for (j=0;j<NUM_DEST_VALUES;j++) { // Destinations number index
		for (i=0;i<NUM_FAULT_VALUES;i++) { // Faults number index
			for (r=FIRST_ALG;r<=LAST_ALG;r++){	// routing alg.: dor, lf, opt, smart
                resrc[j][i][r]=0.0;
                ent[j][i][r]=0.0;
                useX[j][i][r]=0.0;
                useY[j][i][r]=0.0;
                ...
                exec[j][i][r]=0.0;
                post_exec[j][i][r]=0.0;
                disconnected[j][i][r]=0.0;
            }
        }
    }

	for (j=0;j<NUM_DEST_VALUES;j++) { // destination number index
		printf("========================\n%6ld destinations\n========================\n", dests[j]);
		printf("metric,   dest, faults,         dor,          lf,         opt,       smart,graph_search\n");
        for (i=0; i<NUM_FAULT_VALUES; i++) { // fault number index
			for (r=FIRST_ALG;r<=LAST_ALG;r++){	// routing alg.: dor, lf, opt, smart
				// printf("%d\n",r);
				for (k=start_rep;k<end_rep; k++) {
					srand(k);
					create(dests[j],faults[i]);
					#ifdef A_STAR
						a_star_reset();
					#endif
					disconnected2=0;
					...
					switch(r){
						default:
						case 0: dor(dests[j]); break;
						case 1: lf(dests[j]); break;
						case 2: opt(dests[j]); break;
						case 3: smart(dests[j],RANGE); break;
						#ifdef A_STAR
						case 4: just_a_star(dests[j]); break;
						#endif
					//	default: break;
					}
					gettimeofday(&end_time,NULL);
						...
						if (a_star_required)
							a_star();
					#endif
					gettimeofday(&post_end_time,NULL);

					// Include the value 'value' in the result variable 'measure',
					// including updating the min/max
					#define RECORD(measure, value) do { \
						long double _value = value; \
						measure[j][i][r] += _value; \
						measure ## _sq[j][i][r] += (_value)*(_value); \
						if (k == 0) { \
							measure ## _min[j][i][r] = _value; \
							measure ## _max[j][i][r] = _value; \
						} else { \
							if (value < measure ## _min[j][i][r]) \
								measure ## _min[j][i][r] = _value; \
							if (value > measure ## _max[j][i][r]) \
								measure ## _max[j][i][r] = _value; \
						} \
					} while (0)

					RECORD(resrc, resources());
					RECORD(ent, entries());
					RECORD(useX, dir(1));
					RECORD(useY, dir(2));
					RECORD(useD, dir(4));
					RECORD(exec, (1000000.0 * (end_time.tv_sec-start_time.tv_sec)) + (end_time.tv_usec-start_time.tv_usec));
					RECORD(post_exec, (1000000.0 * (post_end_time.tv_sec-end_time.tv_sec)) + (post_end_time.tv_usec-end_time.tv_usec));
					RECORD(disconnected, disconnected2);
//					exit(-1);
				} //k: repetitions (samples)
				// time(&end_time);
//print_map();
                resrc_stdev[j][i][r]=sqrt(fabs((resrc_sq[j][i][r]-(resrc[j][i][r]*resrc[j][i][r])/rep)/(rep-1)));
                ent_stdev[j][i][r]=sqrt(fabs((ent_sq[j][i][r]-(ent[j][i][r]*ent[j][i][r])/rep)/(rep-1)));
                useX_stdev[j][i][r]=sqrt(fabs((useX_sq[j][i][r]-(useX[j][i][r]*useX[j][i][r])/rep)/(rep-1)));
                useY_stdev[j][i][r]=sqrt(fabs((useY_sq[j][i][r]-(useY[j][i][r]*useY[j][i][r])/rep)/(rep-1)));
                useD_stdev[j][i][r]=sqrt(fabs((useD_sq[j][i][r]-(useD[j][i][r]*useD[j][i][r])/rep)/(rep-1)));
                exec_stdev[j][i][r]=sqrt(fabs((exec_sq[j][i][r]-(exec[j][i][r]*exec[j][i][r])/rep)/(rep-1)));
                post_exec_stdev[j][i][r]=sqrt(fabs((post_exec_sq[j][i][r]-(post_exec[j][i][r]*post_exec[j][i][r])/rep)/(rep-1)));
                disconnected_stdev[j][i][r]=sqrt(fabs((disconnected_sq[j][i][r]-(disconnected[j][i][r]*disconnected[j][i][r])/rep)/(rep-1)));
				...
				exec[j][i][r]/=rep;
				post_exec[j][i][r]/=rep;
				disconnected[j][i][r]/=rep;
			}//r: routing algorithm

			printf("resource, %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
					resrc[j][i][0], resrc[j][i][1], resrc[j][i][2], resrc[j][i][3], resrc[j][i][4]);
			printf("resource_stdev, %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
					resrc_stdev[j][i][0], resrc_stdev[j][i][1], resrc_stdev[j][i][2], resrc_stdev[j][i][3], resrc_stdev[j][i][4]);
			printf("resource_min, %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
					resrc_min[j][i][0], resrc_min[j][i][1], resrc_min[j][i][2], resrc_min[j][i][3], resrc_min[j][i][4]);
			printf("resource_max, %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
					resrc_max[j][i][0], resrc_max[j][i][1], resrc_max[j][i][2], resrc_max[j][i][3], resrc_max[j][i][4]);

			printf("entries,  %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    ent[j][i][0], ent[j][i][1], ent[j][i][2], ent[j][i][3], ent[j][i][4]);
			printf("entries_stdev,  %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    ent_stdev[j][i][0], ent_stdev[j][i][1], ent_stdev[j][i][2], ent_stdev[j][i][3], ent_stdev[j][i][4]);
            printf("entries_min,  %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    ent_min[j][i][0], ent_min[j][i][1], ent_min[j][i][2], ent_min[j][i][3], ent_min[j][i][4]);
			printf("entries_max,  %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    ent_max[j][i][0], ent_max[j][i][1], ent_max[j][i][2], ent_max[j][i][3], ent_max[j][i][4]);

			printf("useX,     %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    useX[j][i][0], useX[j][i][1], useX[j][i][2], useX[j][i][3], useX[j][i][4]);
			printf("useX_stdev,     %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    useX_stdev[j][i][0], useX_stdev[j][i][1], useX_stdev[j][i][2], useX_stdev[j][i][3], useX_stdev[j][i][4]);
			printf("useX_min,     %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    useX_min[j][i][0], useX_min[j][i][1], useX_min[j][i][2], useX_min[j][i][3], useX_min[j][i][4]);
			printf("useX_max,     %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    useX_max[j][i][0], useX_max[j][i][1], useX_max[j][i][2], useX_max[j][i][3], useX_max[j][i][4]);

			printf("useY,     %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    useY[j][i][0], useY[j][i][1], useY[j][i][2], useY[j][i][3], useY[j][i][4]);
			printf("useY_stdev,     %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    useY_stdev[j][i][0], useY_stdev[j][i][1], useY_stdev[j][i][2], useY_stdev[j][i][3], useY_stdev[j][i][4]);
			printf("useY_min,     %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    useY_min[j][i][0], useY_min[j][i][1], useY_min[j][i][2], useY_min[j][i][3], useY_min[j][i][4]);
			printf("useY_max,     %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    useY_max[j][i][0], useY_max[j][i][1], useY_max[j][i][2], useY_max[j][i][3], useY_max[j][i][4]);

			printf("useD,     %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    useD[j][i][0], useD[j][i][1], useD[j][i][2], useD[j][i][3], useD[j][i][4]);
			printf("useD_stdev,     %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    useD_stdev[j][i][0], useD_stdev[j][i][1], useD_stdev[j][i][2], useD_stdev[j][i][3], useD_stdev[j][i][4]);
			printf("useD_min,     %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    useD_min[j][i][0], useD_min[j][i][1], useD_min[j][i][2], useD_min[j][i][3], useD_min[j][i][4]);
			printf("useD_max,     %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    useD_max[j][i][0], useD_max[j][i][1], useD_max[j][i][2], useD_max[j][i][3], useD_max[j][i][4]);

			printf("disconne, %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
					disconnected[j][i][0], disconnected[j][i][1], disconnected[j][i][2], disconnected[j][i][3], disconnected[j][i][4]);
			printf("disconne_stdev, %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
					disconnected_stdev[j][i][0], disconnected_stdev[j][i][1], disconnected_stdev[j][i][2], disconnected_stdev[j][i][3], disconnected_stdev[j][i][4]);
			printf("disconne_min, %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
					disconnected_min[j][i][0], disconnected_min[j][i][1], disconnected_min[j][i][2], disconnected_min[j][i][3], disconnected_min[j][i][4]);
			printf("disconne_max, %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
					disconnected_max[j][i][0], disconnected_max[j][i][1], disconnected_max[j][i][2], disconnected_max[j][i][3], disconnected_max[j][i][4]);

			printf("exectime, %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    exec[j][i][0], exec[j][i][1], exec[j][i][2], exec[j][i][3], exec[j][i][4]);
			printf("exectime_stdev, %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    exec_stdev[j][i][0], exec_stdev[j][i][1], exec_stdev[j][i][2], exec_stdev[j][i][3], exec_stdev[j][i][4]);
			printf("exectime_min, %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    exec_min[j][i][0], exec_min[j][i][1], exec_min[j][i][2], exec_min[j][i][3], exec_min[j][i][4]);
			printf("exectime_max, %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    exec_max[j][i][0], exec_max[j][i][1], exec_max[j][i][2], exec_max[j][i][3], exec_max[j][i][4]);

			printf("post_exectime, %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    post_exec[j][i][0], post_exec[j][i][1], post_exec[j][i][2],
                    post_exec[j][i][3], post_exec[j][i][4]);
			printf("post_exectime_stdev, %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    post_exec_stdev[j][i][0], post_exec_stdev[j][i][1],
                    post_exec_stdev[j][i][2], post_exec_stdev[j][i][3], post_exec_stdev[j][i][4]);
			printf("post_exectime_min, %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    post_exec_min[j][i][0], post_exec_min[j][i][1],
                    post_exec_min[j][i][2], post_exec_min[j][i][3], post_exec_min[j][i][4]);
			printf("post_exectime_max, %4ld, %6ld, %11.5LG, %11.5LG, %11.5LG, %11.5LG, %11.5LG\n",dests[j],faults[i],
                    post_exec_max[j][i][0], post_exec_max[j][i][1],
                    post_exec_max[j][i][2], post_exec_max[j][i][3], post_exec_max[j][i][4]);

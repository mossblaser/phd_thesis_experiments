...
# Number of routes to try per failiure mode and traffic pattern
REPEATS=10000

# Number of repeitions to run in each instance of the program
REPEATS_PER_RUN=100
...
# Determine the most appropriate cluster management application to use
if [ -n "$cluster_file" ]; then
	# If a list of hosts was given, run the experiments on that cluster
	parallel_script="clstr -s -vv -A active_nodes -N $cluster_file -J"
...
$parallel_script <(
	for repeat_num in `seq 0 $REPEATS_PER_RUN $(($REPEATS - 1))`; do
		for f in experiment.*; do
			[ -x "$f" ] && echo 'cd '"$PWD"'; ./'"$f"' '$repeat_num' '$(( $repeat_num + $REPEATS_PER_RUN ))' | gzip > "output.'"$f"','$repeat_num'"'
		done
	done
)
...
(
	# Print the header
	echo -n "experiment,fault_model,dist,postprocess,repeat_group,num_samples,"
	gunzip $(ls output.experiment.* | head -n1) -c | grep -E '^metric' | head -n1
	
	# Print the body of the data
	for f in output.experiment.*; do
		gunzip "$f" -c \
			| grep -vE '====|^ |^metric,|^[ ]*$' \
			| sed -re "s/^(.*)$/${f:18},${REPEATS_PER_RUN},\\1/"
	done
) | gzip > output.csv.gz
